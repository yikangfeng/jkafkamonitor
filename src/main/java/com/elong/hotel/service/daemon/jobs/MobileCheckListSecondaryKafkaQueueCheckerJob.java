...
public class MobileCheckListSecondaryKafkaQueueCheckerJob extends AbstractJob implements InitializingBean, Job {
	static private final Logger LOGGER = LoggerFactory.getLogger(MobileCheckListSecondaryKafkaQueueCheckerJob.class);
	@Value("${mobile.checklist.secondary.kafka.zookeeper.urls}")
	private String zookeeperUrls;
	@Value("${mobile.checklist.secondary.kafka.queue.checker.alert.name}")
	private String alertName;
	@Value("${mobile.checklist.secondary.kafka.queue.checker.threshold}")
	private long checkerThreshold;
	@Value("${mobile.checklist.secondary.kafka.queue.checker.consumer.group}")
	private String checkerConsumerGroup;
	@Value("${mobile.checklist.secondary.kafka.queue.checker.topic}")
	...
	public MobileCheckListSecondaryKafkaQueueCheckerJob() {
		this.ALERT_MSG_TEMPLATE = "%s当前mobile-checklist-kafka_secondary待消费消息为%d条超过阈值%d";
	}
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			offsetMonitorList = SpringUtils.getBean(MobileCheckListSecondaryKafkaQueueCheckerJob.class).kafkaConsumerOffsetUtil
			...
			if (totalLag > this.checkerThreshold) {
				...
				LOGGER.info(String.format("%s当前mobile-checklist-kafka_secondary %d条小于阈值%d",
						new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()), totalLag, checkerThreshold));
			}
		} catch (Throwable t) {
			// TODO Auto-generated catch block
			LOGGER.error("The current job={} mobile-checklist-kafka_secondary queue checker job execute failed,reasons:",
					jobName(), t);
		} finally {
	...
	protected String jobName() {
		// TODO Auto-generated method stub
		return MobileCheckListSecondaryKafkaQueueCheckerJob.class.getName();
	}

	@Override
	protected String jobGroup() {
		// TODO Auto-generated method stub
		return "MOBILE_CHECKLIST_SECONDARY_QUEUE_CHECK_GROUP";
	}
	...
	protected String triggerName() {
		// TODO Auto-generated method stub
		return "MOBILE_CHECKLIST_SECONDARY_QUEUE_CHECK_TRIGGER";
	}

	@Override
	protected String triggerGroup() {
		// TODO Auto-generated method stub
		return "MOBILE_CHECKLIST_SECONDARY_QUEUE_CHECK_TRIGGER_GROUP";
	}
