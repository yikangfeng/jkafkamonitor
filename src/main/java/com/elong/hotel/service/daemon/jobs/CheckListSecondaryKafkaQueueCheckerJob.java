...
public class CheckListSecondaryKafkaQueueCheckerJob extends AbstractJob implements InitializingBean, Job {
	static private final Logger LOGGER = LoggerFactory.getLogger(CheckListSecondaryKafkaQueueCheckerJob.class);
	@Value("${checklist.secondary.kafka.zookeeper.urls}")
	private String zookeeperUrls;
	@Value("${checklist.secondary.kafka.queue.checker.alert.name}")
	private String alertName;
	@Value("${checklist.secondary.kafka.queue.checker.threshold}")
	private long checkerThreshold;
	@Value("${checklist.secondary.kafka.queue.checker.consumer.group}")
	private String checkerConsumerGroup;
	@Value("${checklist.secondary.kafka.queue.checker.topic}")
	...
	public CheckListSecondaryKafkaQueueCheckerJob() {
		this.ALERT_MSG_TEMPLATE = "%s当前hotel-checklist-kafka_secondary待消费消息为%d条超过阈值%d";
	}
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			offsetMonitorList = SpringUtils
					.getBean(CheckListSecondaryKafkaQueueCheckerJob.class).kafkaConsumerOffsetUtil
			...
			if (totalLag > this.checkerThreshold) {
				...
				LOGGER.info(String.format("%s当前hotel-checklist-kafka_secondary %d条小于阈值%d",
						new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()), totalLag, checkerThreshold));
			}
		} catch (Throwable t) {
			// TODO Auto-generated catch block
			LOGGER.error("The current job={} hotel-checklist-kafka_secondary queue checker job execute failed,reasons:",
					jobName(), t);
		} finally {
	...
	protected String jobName() {
		// TODO Auto-generated method stub
		return CheckListSecondaryKafkaQueueCheckerJob.class.getName();
	}

	@Override
	protected String jobGroup() {
		// TODO Auto-generated method stub
		return "CHECKLIST_SECONDARY_QUEUE_CHECK_GROUP";
	}
	...
	protected String triggerName() {
		// TODO Auto-generated method stub
		return "CHECKLIST_SECONDARY_QUEUE_CHECK_TRIGGER";
	}

	@Override
	protected String triggerGroup() {
		// TODO Auto-generated method stub
		return "CHECKLIST_SECONDARY_QUEUE_CHECK_TRIGGER_GROUP";
	}
