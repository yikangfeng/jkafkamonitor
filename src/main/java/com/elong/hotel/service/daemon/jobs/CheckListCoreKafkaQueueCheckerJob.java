...
public class CheckListCoreKafkaQueueCheckerJob extends AbstractJob implements InitializingBean, Job {
	static private final Logger LOGGER = LoggerFactory.getLogger(CheckListCoreKafkaQueueCheckerJob.class);
	@Value("${checklist.core.kafka.zookeeper.urls}")
	private String zookeeperUrls;
	@Value("${checklist.core.kafka.queue.checker.alert.name}")
	private String alertName;
	@Value("${checklist.core.kafka.queue.checker.threshold}")
	private long checkerThreshold;
	@Value("${checklist.core.kafka.queue.checker.consumer.group}")
	private String checkerConsumerGroup;
	@Value("${checklist.core.kafka.queue.checker.topic}")
	private String checkerTopic;
	private KafkaConsumerOffsetUtil kafkaConsumerOffsetUtil;

	public CheckListCoreKafkaQueueCheckerJob() {
		this.ALERT_MSG_TEMPLATE = "%s当前hotel-checklist-kafka_core待消费消息为%d条超过阈值%d";
	}
	...
	public void afterPropertiesSet() throws Exception {
		...
		Runtime.getRuntime().addShutdownHook(new Thread(() -> {
			try {
				...
				LOGGER.error("The current job={} closing zkclient error,reasons", this.getClass().getSimpleName(), t);
			}
		}));
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			offsetMonitorList = SpringUtils.getBean(CheckListCoreKafkaQueueCheckerJob.class).kafkaConsumerOffsetUtil
			...
			if (totalLag > this.checkerThreshold) {
				...
				LOGGER.info(String.format("%s当前hotel-checklist-kafka_core %d条小于阈值%d",
						new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()), totalLag, checkerThreshold));
			}
		} catch (Throwable t) {
			// TODO Auto-generated catch block
			LOGGER.error("The current job={} hotel-checklist-kafka_core queue checker job execute failed,reasons:", jobName(),
					t);
		} finally {
	...
	protected String jobName() {
		// TODO Auto-generated method stub
		return CheckListCoreKafkaQueueCheckerJob.class.getName();
	}

	@Override
	protected String jobGroup() {
		// TODO Auto-generated method stub
		return "CHECKLIST_CORE_QUEUE_CHECK_GROUP";
	}
	...
	protected String triggerName() {
		// TODO Auto-generated method stub
		return "CHECKLIST_CORE_QUEUE_CHECK_TRIGGER";
	}

	@Override
	protected String triggerGroup() {
		// TODO Auto-generated method stub
		return "CHECKLIST_CORE_QUEUE_CHECK_TRIGGER_GROUP";
	}
