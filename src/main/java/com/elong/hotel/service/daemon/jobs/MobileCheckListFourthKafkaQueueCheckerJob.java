...
public class MobileCheckListFourthKafkaQueueCheckerJob extends AbstractJob implements InitializingBean, Job {
	static private final Logger LOGGER = LoggerFactory.getLogger(MobileCheckListFourthKafkaQueueCheckerJob.class);
	@Value("${mobile.checklist.fourth.kafka.zookeeper.urls}")
	private String zookeeperUrls;
	@Value("${mobile.checklist.fourth.kafka.queue.checker.alert.name}")
	private String alertName;
	@Value("${mobile.checklist.fourth.kafka.queue.checker.threshold}")
	private long checkerThreshold;
	@Value("${mobile.checklist.fourth.kafka.queue.checker.consumer.group}")
	private String checkerConsumerGroup;
	@Value("${mobile.checklist.fourth.kafka.queue.checker.topic}")
	...
	public MobileCheckListFourthKafkaQueueCheckerJob() {
		this.ALERT_MSG_TEMPLATE = "%s当前mobile-checklist-kafka_fourth待消费消息为%d条超过阈值%d";
	}
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			offsetMonitorList = SpringUtils.getBean(MobileCheckListFourthKafkaQueueCheckerJob.class).kafkaConsumerOffsetUtil
			...
			if (totalLag > this.checkerThreshold) {
				...
				LOGGER.info(String.format("%s当前mobile-checklist-kafka_fourth %d条小于阈值%d",
						new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()), totalLag, checkerThreshold));
			}
		} catch (Throwable t) {
			// TODO Auto-generated catch block
			LOGGER.error("The current job={} mobile-checklist-kafka_fourth queue checker job execute failed,reasons:",
					jobName(), t);
		} finally {
	...
	protected String jobName() {
		// TODO Auto-generated method stub
		return MobileCheckListFourthKafkaQueueCheckerJob.class.getName();
	}

	@Override
	protected String jobGroup() {
		// TODO Auto-generated method stub
		return "MOBILE_CHECKLIST_FOURTH_QUEUE_CHECK_GROUP";
	}
	...
	protected String triggerName() {
		// TODO Auto-generated method stub
		return "MOBILE_CHECKLIST_FOURTH_QUEUE_CHECK_TRIGGER";
	}

	@Override
	protected String triggerGroup() {
		// TODO Auto-generated method stub
		return "MOBILE_CHECKLIST_FOURTH_QUEUE_CHECK_TRIGGER_GROUP";
	}
